...
$pdf->SetX(4);            
$pdf->MultiCell(24.5,0.5,'Bengkel Abror Motor Kendal',0,'L');
$pdf->SetX(4);   
...
$pdf->Cell(4, 0.8, 'Jumlah Stok Keluar', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
$query=mysql_query("select * from stok_keluar");
while($lihat=mysql_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['Tanggal'],1, 0, 'C');
	$pdf->Cell(7, 0.8, $lihat['Kode_Barang'], 1, 0,'C');
	$pdf->Cell(10, 0.8, $lihat['Nama_Barang'], 1, 0,'C');
	$pdf->Cell(4, 0.8, $lihat['Jumlah'],1, 1, 'C');

	$no++;
}

$pdf->Output("cetak_stok.pdf","I");
